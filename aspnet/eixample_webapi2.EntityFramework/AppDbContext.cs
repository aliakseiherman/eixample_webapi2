...
namespace eixample_webapi2.EntityFramework
{
    public class AppDbContext : IdentityDbContext<ApplicationUser>
    {
        ...
        private void AuditEntities()
        {
            ...
            foreach (var entity in ChangeTracker.Entries<IHasCreator>().Where(p => p.State == EntityState.Added).Select(p => p.Entity))
            {
            ...
            foreach (var entity in ChangeTracker.Entries<IHasModifier>().Where(p => p.State == EntityState.Modified).Select(p => p.Entity))
            {
